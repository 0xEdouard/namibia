...
# Create map centered on Namibia
...
PolyLine(route_coords, color='blue', weight=4, opacity=0.7).add_to(m)

# Save interactive HTML map
m.save('images/route_map.html')

# Also save as screenshot using selenium (if available) or just note
try:
    import io
    from PIL import Image
    # For now, just create a placeholder - screenshot requires selenium + webdriver
    # Users will use the HTML version
    print(r'\textit{Interactive map: images/route\_map.html}')
except:
    print(r'\textit{Map generated successfully}')
\end{pycode}

\begin{center}
\textit{View interactive map: \href{images/route\_map.html}{route\_map.html}}
\end{center}
...
\noindent\textbf{Windhoek} $\rightarrow$ (268 km, 3.5h, C19 gravel) $\rightarrow$ \textbf{Solitaire} [Fuel + Apple Crumble!] $\rightarrow$ (82 km, 1.5h, C27 gravel) $\rightarrow$ \textbf{Sossusvlei}
